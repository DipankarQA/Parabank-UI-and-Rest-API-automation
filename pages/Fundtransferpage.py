...
class Fundtransferpage():
    ...
    def Fundtransfer(self, amount, fromacc, toacc ):
        ...
        print(self.page.locator("//*[text()='Transfer Complete!']").text_content())
        head ={"Accept": "application/xml"}
        response = requests.get(f"https://parabank.parasoft.com/parabank/services/bank/accounts/{toacc}", headers = head  )
        print(response.status_code)
        assert 200 == response.status_code
        print("response text:",response.text)
        #assert toacc in response.text
        return toacc['balance']
